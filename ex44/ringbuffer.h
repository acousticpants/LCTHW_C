...
//ringbuffer loops end of list to beginning
#include <lcthw/bstrlib.h>
//variant of queue data structure good for handline asynchronous IO using blocks of bytes instead of list of pointers
typedef struct {
    char *buffer;
    int length;
    int start;
...
#define RingBuffer_commit_read(B, A) ((B)->start = ((B)>start +  (A)) % (B)_>length)
