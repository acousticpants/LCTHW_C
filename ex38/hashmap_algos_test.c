...
char *test_fnv1a()
{
    ...
    hash = Hashmap_fnv1a_1a_hash(&test2);
    mu_assert(hash != 0, "Bad hash.");
...
int gen_keys(DArray *keys, int num_keys)//makes big list of keys
{
    int i = 0;
...
char *test_distribution()
{
    ...
    for(i = 0; i < BUCKETS; i++) {//prints 3 column table to console showing keys
