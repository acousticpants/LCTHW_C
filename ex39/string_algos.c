...
#include <limits.h>
//string_find and stringscanner_scan both use setup_skip_chars and string_base_search to find the needle
static inline void String_setup_skip_chars(//sets skip list according to nlen length of needle string
    size_t *skip_chars,
...
int String_find(bstring in, bstring what)//basic form of BMH alg finds 1st instance of needle
{
...
int StringScanner_scan(StringScanner *scan, bstring tofind)//more complex BMH alg can be run incrementally to find all instances of needle
{
    const unsigned char *found = NULL;
    ssize_t found_at = 0;
